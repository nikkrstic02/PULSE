...
type LoginFormProps = {
  space: "organize" | "lifestyle";
};

export function LoginForm({ space }: LoginFormProps) {
  const loginMutation = useLoginMutation();
  const router = useRouter();
  const { copy } = useLanguageCopy();
  const authCopy = copy.auth;

  const defaultRedirect = space === "organize" ? "/organize/dashboard" : "/lifestyle/dashboard";
  const otherLoginUrl = space === "organize" ? "/login/lifestyle" : "/login/organize";
  const otherLabel = space === "organize" ? "Pulse Lifestyle" : "Pulse Organize";
  const accent = space === "organize" ? "cyan" : "emerald";
  ...
  const handleGoogleSignIn = () => {
    window.location.assign(getGoogleRedirectUrl(defaultRedirect));
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.replace(next ? getSafeNextPath(next) : defaultRedirect);
    } catch {
  ...
  return (
    <main className="pulse-auth-shell flex min-h-dvh items-center justify-center px-4 py-6 sm:p-6">
      <div className="pulse-glass w-full max-w-[520px] rounded-[18px] px-5 py-7 shadow-[0_20px_60px_rgba(0,0,0,0.35)] sm:min-h-[640px] sm:px-8 sm:py-9">
        <div className="flex flex-col items-center">
          <div className={`pulse-wordmark text-2xl`}>Pulse</div>
          <div className={`mt-2 text-sm font-bold uppercase tracking-[0.18em] text-${accent}-300`}>
            {space === "organize" ? "Organize" : "Lifestyle"}
          </div>
        ...
        <form onSubmit={onSubmit} autoComplete="on" className="mt-7 space-y-4">
          ...
          {googleError ? (
            <div className="pulse-auth-alert rounded-[10px] px-4 py-3 text-sm">
              {googleError}
            </div>
          ) : null}
          ...
          <button
            ...
            className={`w-full rounded-[12px] py-3 text-[15px] font-semibold text-white hover:brightness-110 disabled:opacity-60 ${
              accent === "cyan" ? "bg-cyan-500" : "bg-emerald-500"
            }`}
          >
          ...
          <div className="pt-1 text-center text-sm text-slate-400">
            <Link href={otherLoginUrl} className={`font-semibold text-${accent === "cyan" ? "emerald" : "cyan"}-300 hover:underline`}>
